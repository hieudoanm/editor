...
export const ChessClock: FC = () => {
  ...
  const click = (side: ChessClockSide) => {
    ...
    const newTimer = setInterval(() => {
      if (clock.milliseconds.white === 0 || clock.milliseconds.black === 0) {
        ...
        setClock((previous: ClockState) => {
          const { full, timeControl, current, milliseconds, increment, running } = previous;
          if (current === '')
            return {
              full,
              timeControl,
          ...
          const newCurrentMilliseconds: number = milliseconds[current] - ONE_UNIT;
  ...
  const format = (milliseconds: number): string => {
    ...
    const remainingMilliseconds: string = ((milliseconds % ONE_MINUTE) / 1000).toFixed(1);
  ...
  const convertTimeControlToMillisecondsAndIncrement = (timeControl: string) => {
          ...
          <div className="flex items-center gap-x-4 md:gap-x-8">
            ...
            <Glass.Select
              ...
              onChange={(event) => {
                ...
                const newIncrement: number = parseInt(timeControl.at(1) ?? '', 10);
                setClock(({ full, timeControl, current, milliseconds, increment, running }: ClockState) => {
                  const side = 'black';
          ...
          <div className="order-3 md:order-1">
            <div className="grid h-full grid-cols-1 md:grid-cols-2">
              <div className="col-span-1">
                <div className="flex h-full w-full items-center justify-center bg-neutral-900 p-1 text-center text-neutral-100 md:p-2">
                  <div className="rotate-90 md:rotate-0">{clock.timeControl.white}</div>
                </div>
              </div>
              <div className="col-span-1">
                <div className="flex h-full w-full items-center justify-center bg-neutral-100 p-1 text-center text-neutral-900 md:p-2">
                  <div className="rotate-90 md:rotate-0">{clock.timeControl.black}</div>
                </div>
              </div>
            </div>
          </div>
            ...
            <div className="grid h-full grid-cols-1 md:grid-cols-2">
              <div className="col-span-1 row-span-1">
                <button
                  ...
                  onClick={() => click('white')}>
                  <div className="rotate-90 text-5xl md:rotate-0 md:text-9xl">{format(clock.milliseconds.white)}</div>
                </button>
              </div>
              <div className="col-span-1 row-span-1">
                <button
                  ...
                  onClick={() => click('black')}>
                  <div className="rotate-90 text-5xl md:rotate-0 md:text-9xl">{format(clock.milliseconds.black)}</div>
                </button>
              </div>
            </div>
          </div>
              ...
              <button
                ...
                onClick={() =>
                  setClock((previous) => {
                    ...
                    const { white: whiteTimeControl, black: blackTimeControl } = timeControl;
                    const { milliseconds: whiteMilliseconds, increment: whiteIncrement } =
                      convertTimeControlToMillisecondsAndIncrement(whiteTimeControl);
                    const { milliseconds: blackMilliseconds, increment: blackIncrement } =
                      convertTimeControlToMillisecondsAndIncrement(blackTimeControl);
